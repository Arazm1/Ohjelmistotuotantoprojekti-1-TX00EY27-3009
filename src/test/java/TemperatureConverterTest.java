...
class TemperatureConverterTest {
    ...
    @Test
    void testKelvinToCelsiusMultipleValues(){
        double[] kelvinValues = {273.15, 293.15, 311.53, 308.48};
        double[] celsiusValues = {0, 20, 38.38, 35.33};

        for(int i = 0; i<kelvinValues.length; i++){
            double testResult = temperatureConverter.kelvinToCelsius(kelvinValues[i]);
            assertEquals(celsiusValues[i], testResult, allowedDifference);
        }
    }
